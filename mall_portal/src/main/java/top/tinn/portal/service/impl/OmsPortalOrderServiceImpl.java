...
import top.tinn.mapper.*;
import top.tinn.model.*;
import top.tinn.portal.component.CancelOrderSender;
import top.tinn.portal.dao.PortalOrderDao;
import top.tinn.portal.dao.PortalOrderItemDao;
import top.tinn.portal.dao.SmsCouponHistoryDao;
import top.tinn.portal.domain.*;
import top.tinn.portal.service.*;
...
/**
 * @Description OmsPortalOrderServiceImpl
 * @Author Tinn
...
public class OmsPortalOrderServiceImpl implements OmsPortalOrderService {
     ...
     * 计算购物车中商品的价格
     */
    private ConfirmOrderResult.CalcAmount calcCartAmount(
    ...
    public CommonResult generateOrder(OrderParam orderParam) {
        ...
        if (orderParam.getUseIntegration() == null) {
            ...
            BigDecimal integrationAmount =
                    getUseIntegrationAmount(orderParam.getUseIntegration(),
                            totalAmount, currentMember,
        ...
        //收货人信息：姓名、电话、邮编、地址
        UmsMemberReceiveAddress address = memberReceiveAddressService
                .getItem(orderParam.getMemberReceiveAddressId());
        ...
        for (OmsOrderItem orderItem : orderItemList) {
            if (orderItem.getIntegrationAmount() != null) {
                integrationAmount = integrationAmount
                        .add(orderItem.getIntegrationAmount()
        ...
        for (OmsOrderItem orderItem : orderItemList) {
            if (orderItem.getPromotionAmount() != null) {
                promotionAmount = promotionAmount.
                        add(orderItem.getPromotionAmount().
                                multiply(new BigDecimal(orderItem.getProductQuantity())));
            }
        }
    ...
    private void lockStock(List<CartPromotionItem> cartPromotionItemList) {
        for (CartPromotionItem cartPromotionItem : cartPromotionItemList){
            PmsSkuStock skuStock = skuStockMapper
                    .selectByPrimaryKey(cartPromotionItem.getProductSkuId());
    ...
    /**
     * 计算order_item的实付金额,写入RealAmount中
     * @param orderItemList
     */
    private void handleRealAmount(List<OmsOrderItem> orderItemList) {
        for (OmsOrderItem orderItem : orderItemList){
            //原价-促销优惠-优惠券抵扣-积分抵扣
            BigDecimal readAmount = orderItem.getProductPrice()
                    .subtract(orderItem.getPromotionAmount())
                    .subtract(orderItem.getCouponAmount())
                    .subtract(orderItem.getIntegrationAmount());
            orderItem.setRealAmount(readAmount);
        }
    }
    ...
    private BigDecimal getUseIntegrationAmount(Integer useIntegration,
        ...
        UmsIntegrationConsumeSetting integrationConsumeSetting =
                integrationConsumeSettingMapper.selectByPrimaryKey(1L);
        ...
        BigDecimal integrationAmount = new BigDecimal(useIntegration)
                .divide(new BigDecimal(integrationConsumeSetting.getUseUnit()),
                        2, RoundingMode.HALF_EVEN);
        BigDecimal maxPercent = new BigDecimal(integrationConsumeSetting
                .getMaxPercentPerOrder()).divide(new BigDecimal(100),
                2, RoundingMode.HALF_EVEN);
    ...
    private List<OmsOrderItem> getCouponOrderItemByRelation(
        ...
        if (type == 0){
            ...
            for (OmsOrderItem orderItem : orderItemList){
                ...
                else orderItem.setCouponAmount(new BigDecimal(0));
            }
        }else if (type == 1){
            List<Long> productIdList = new ArrayList<>();
            for (SmsCouponProductRelation productRelation
                    : couponHistoryDetail.getProductRelationList()) {
     ...
     */
    private void calcPerCouponAmount(List<OmsOrderItem> orderItemList,
                                     SmsCoupon coupon) {
        BigDecimal totalAmount = calcTotalAmount(orderItemList);
        for (OmsOrderItem orderItem : orderItemList){
            //(商品价格/可用商品总价)*优惠券面额
    ...
    private SmsCouponHistoryDetail getUseCoupon(
            List<CartPromotionItem> cartPromotionItemList, Long couponId) {
        List<SmsCouponHistoryDetail> couponHistoryDetailList = memberCouponService
                .listCart(cartPromotionItemList,1);
    ...
    private boolean hasStock(List<CartPromotionItem> cartPromotionItemList) {
        for (CartPromotionItem item : cartPromotionItemList){
            if (item.getRealStock() == null || item.getRealStock() <= 0) return false;
        }
        return true;
    }
    ...
    public CommonResult cancelTimeOutOrder() {
        ...
        //查询超时、未支付的订单及订单详情
        List<OmsOrderDetail> timeOutOrders = portalOrderDao
                .getTimeOutOrders(orderSetting.getNormalOrderOvertime());
        ...
        for (OmsOrderDetail timeOutOrder : timeOutOrders){
            ...
            //修改优惠券使用状态
            updateCouponStatus(timeOutOrder.getCouponId(),
                    timeOutOrder.getMemberId(),0);
        }
        LOGGER.info("修改超时订单STATUS->4,订单SN号为:{}",timeOutOrders.stream().map(timeOutOrder->timeOutOrder.getOrderSn()).collect(Collectors.toList()).toString());
    ...
    public void cancelOrder(Long orderId) {
        ...
        example.createCriteria().andIdEqualTo(orderId)
                .andStatusEqualTo(0).andDeleteStatusEqualTo(0);
        List<OmsOrder> cancelOrderList = orderMapper
        ...
        if (cancelOrder != null) {
            ...
            List<OmsOrderItem> orderItemList = orderItemMapper
                    .selectByExample(orderItemExample);
            //解除订单商品库存锁定
            if (!CollectionUtils.isEmpty(orderItemList)) {
                portalOrderDao.releaseSkuStockLock(orderItemList);
            }
            //修改优惠券使用状态
            updateCouponStatus(cancelOrder.getCouponId(),
                    cancelOrder.getMemberId(), 0);
            //返还使用积分
